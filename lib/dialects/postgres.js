...
exports.postgresOptions = {
  ...
  showElementTypeQuery: (tableName, schemaName) => {
    ...
    (SELECT array_agg(pe.enumlabel) FROM pg_catalog.pg_type pt JOIN pg_catalog.pg_enum pe ON pt.oid=pe.enumtypid 	    
      WHERE (pt.typname=c.udt_name OR CONCAT('_',pt.typname)=c.udt_name) and pt.typtype='e' and pe.enumtypid = concat('${schemaName}','.',pt.typname)::regtype::oid ) AS enum_values 
